    ...
    def run(self, state: TradingState):
        ...
        for product in ["RAINFOREST_RESIN", "KELP"]:
            ...
            if product == "RAINFOREST_RESIN":
                ...
                elif best_bid + 1 < 10000:
                    orders.append(Order(product, best_bid + 1, qty))
                else:
                    orders.append(Order(product, 9997, qty))
                qty = min(trade_size, max_sellable)
                if best_bid > 10000:
                    orders.append(Order(product, best_bid, -qty))
                elif best_ask - 1 > 10000:
                    orders.append(Order(product, best_ask - 1, -qty))
                else:
                    orders.append(Order(product, 10003, -qty))
            else:
                qty = min(trade_size, max_buyable)
                if ema_diff > 0:
                    if best_ask < short_ema:
                        orders.append(Order(product, best_ask, qty))
                    elif best_bid + 1 < short_ema:
                        orders.append(Order(product, best_bid + 1, qty))
                qty = min(trade_size, max_sellable)
                if ema_diff < 0:
                    if best_bid > short_ema:
                        orders.append(Order(product, best_bid, -qty))
                    elif best_ask - 1 > short_ema:
                        orders.append(Order(product, best_ask - 1, -qty))
