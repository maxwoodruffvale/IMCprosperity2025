...
# Changelog
# flipped mm_as and bid to min or max
...
class Trader:
    ...
    def compute_volatility(self, past_prices: List[float]) -> float:
        """
        Computes a simple volatility score from a list of past prices
        by taking the standard deviation of consecutive price differences.
        Returns 1.0 if there is not enough data to compute a meaningful score.
        """
        if len(past_prices) < 2:
            return 1.0  # Fallback if we have too few data points

        # Differences between consecutive prices
        diffs = []
        for i in range(len(past_prices) - 1):
            diffs.append(past_prices[i+1] - past_prices[i])

        # Standard deviation of those diffs
        std_dev = np.std(diffs)
        # You can clamp or set a minimum to avoid zero volatility
        return max(std_dev, 0.01)  # for safety
    ...
    def ink(self, state: TradingState, limit: int, stored_data):
        ...
        fair_value = (mm_ask + mm_bid) / 2
        if "past_prices" not in stored_data["SQUID_INK"].keys():
            stored_data["SQUID_INK"]["past_prices"] = []
        
        stored_data["SQUID_INK"]["past_prices"].append(fair_value)
        ...
        # MOVE TS DOWN
        base_take_width = 1
        volatility_score = self.compute_volatility(stored_data["SQUID_INK"]["past_prices"])
        raw_take_width = base_take_width * volatility_score
        min_take_width = 0.5
        max_take_width = 2.5
        take_width = min(max(raw_take_width, min_take_width), max_take_width)
        ...
        stored_data["SQUID_INK"]["last_price"] = forecast_price #fair_value
