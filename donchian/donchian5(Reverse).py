    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if len(history[product]) == LOOKBACK_PERIOD:
                ...
                # Reversed logic:
                # When price >= channel high, we SELL using best bid.
                if current_price >= channel_high:
                    best_bid = self.get_best_bid(order_depth)
                    if best_bid is not None:
                        best_bid_amount = order_depth.buy_orders[best_bid]
                        print("Reversed Breakout UP! Placing SELL order for", -best_bid_amount, "units at", best_bid)
                        orders.append(Order(product, best_bid, -best_bid_amount))
                # When price <= channel low, we BUY using best ask.
                elif current_price <= channel_low:
                    best_ask = self.get_best_ask(order_depth)
                    if best_ask is not None:
                        best_ask_amount = order_depth.sell_orders[best_ask]
                        print("Reversed Breakout DOWN! Placing BUY order for", -best_ask_amount, "units at", best_ask)
