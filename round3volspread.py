...
class Trader:
    ...
    def bs(self, S, K, r, T, sigma):
        N = NormalDist().cdf
        d1 = (math.log(S/K) + (r + 0.5 * (sigma ** 2) * T)) / (sigma * math.sqrt(T))
        d2 = d1 - sigma * math.sqrt(T)
        delta = N(d1)
        return S * delta - K * math.exp(-r * T) * N(d2)

    # calculate implied volatility
    def iv(self, S, K, r, tau, v_fair_value, tol=1e-6, max_iter=100):
        sigma_low = 1e-6
        sigma_high = 5.0

        for _ in range(max_iter):
            sigma_mid = (sigma_low + sigma_high) / 2
            price_mid = self.bs(S, K, r, tau, sigma_mid)

            if abs(price_mid - v_fair_value) < tol:
                return sigma_mid
            elif price_mid < v_fair_value:
                sigma_low = sigma_mid
            else:
    ...
    def v9500_strategy(
        ...
        T = (7 - timestamp / 10000) / 365
        sigma = 0.189570942305608
        iv = self.iv(S, K, r, T, fair_value)

        # obtain RV
        rv_window = 50
        stored_data["VOLCANIC_ROCK"]["log_prices"].append(np.log(rock_fair_value))
        if len(stored_data["VOLCANIC_ROCK"]["log_prices"]) < 5:
            return orders
        elif len(stored_data["VOLCANIC_ROCK"]["log_prices"]) > rv_window:
            stored_data["VOLCANIC_ROCK"]["log_prices"].pop(0)

        log_prices = np.array(stored_data["VOLCANIC_ROCK"]["log_prices"])
        log_returns = np.diff(log_prices)

        rv = log_returns.std() * np.sqrt(365 * 10000)

        # obtain normalized vol spread
        # avg iv - rv = -0.26753097236, avg iv = 0.189570942305608, avg rv = 0.4571019146718725
        vol_spread = iv - rv + 0.26753097236

        # obtain z score
        z_window = 50
        z_threshold = 2
        stored_data["VOLCANIC_ROCK_VOUCHER_9500"]["spread_history"].append(vol_spread)
        if len(stored_data["VOLCANIC_ROCK_VOUCHER_9500"]["spread_history"]) < 5:
            return orders
        elif len(stored_data["VOLCANIC_ROCK_VOUCHER_9500"]["spread_history"]) > z_window:
            stored_data["VOLCANIC_ROCK_VOUCHER_9500"]["spread_history"].pop(0)

        vol_spread_std = np.std(stored_data["VOLCANIC_ROCK_VOUCHER_9500"]["spread_history"])

        z_score = vol_spread / vol_spread_std
        
        logger.print(iv, rv, vol_spread, z_score)
        ...
        # target_position = - round(position * delta)
        #
        # rebalance_threshold = 80
        #
        # diff = target_position - rock_position
        #
        # if abs(diff) > rebalance_threshold:
        #     rock_best_bid = max(rock_order_depth.buy_orders.keys())
        #     rock_best_ask = min(rock_order_depth.sell_orders.keys())
        #     if diff > 0:
        #         # We want to buy 'diff'
        #         possible_buy = -rock_order_depth.sell_orders[rock_best_ask]  # volume at best ask
        #         volume = min(diff, possible_buy, rock_limit - rock_position)
        #         if volume > 0:
        #             rock_orders.append(Order("VOLCANIC_ROCK", rock_best_ask, volume))
        #     elif diff < 0:
        #         possible_sell = rock_order_depth.buy_orders[rock_best_bid]
        #         volume = min(-diff, possible_sell, rock_position + limit)
        #         if volume > 0:
        #             rock_orders.append(Order("VOLCANIC_ROCK", rock_best_bid, -volume))
        ...
        for product in ["RAINFOREST_RESIN",
            ...
            if product == "VOLCANIC_ROCK_VOUCHER_9500":
                result["VOLCANIC_ROCK_VOUCHER_9500"] = self.v9500_strategy(state,
                                                          POSITION_LIMITS["VOLCANIC_ROCK_VOUCHER_9500"],
