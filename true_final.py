...
class Trader:
    ...
    def ink(self, state: TradingState, limit: int, stored_data):
        ...
        asks_above_pred = [p for p in order_depth.sell_orders if p > predicted_price*.5  + fair_value*.5+ disregard_edge]
        bids_below_pred = [p for p in order_depth.buy_orders if p < predicted_price*.5  + fair_value*.5 - disregard_edge]
    ...
    def run(self, state: TradingState):
        stored_data = jsonpickle.loads(state.traderData) if state.traderData else {}
        ...
        trader_data = jsonpickle.dumps(stored_data, separators = (",", ":"))
